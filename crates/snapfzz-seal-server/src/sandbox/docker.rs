...
impl DockerBackend {
    ...
    pub async fn provision(&self, config: &SandboxConfig) -> Result<SandboxHandle, SealError> {
        ...
        backend_data.insert("container_id".to_string(), container_id.clone());
    ...
    pub async fn copy_into(
        ...
        let container_id = handle.container_id.clone().ok_or_else(|| {
            SealError::InvalidInput("sandbox handle has no container id".to_string())
        })?;
...
impl SandboxBackend for DockerBackend {
    async fn provision(&self, config: &SandboxConfig) -> Result<SandboxHandle, SealError> {
        DockerBackend::provision(self, config).await
    }
    ...
    async fn copy_into(
        ...
        DockerBackend::copy_into(self, handle, host_path, target).await
    }
    ...
    async fn exec(
        ...
        DockerBackend::exec(self, handle, command, timeout_secs).await
    }

    async fn destroy(&self, handle: &SandboxHandle) -> Result<(), SealError> {
        DockerBackend::destroy(self, handle).await
    }
...
mod tests {
    ...
    use crate::sandbox::{SandboxConfig, SandboxHandle, copy_into_sandbox, exec_in_sandbox};

    use super::{
        DockerBackend, find_docker_binary, is_executable, new_sandbox_id, parse_container_id,
        random_hex_4, unix_ts_millis, validate_sandbox_config,
    };
    ...
    fn run_env_probe(mode: &str, envs: &[(&str, &str)]) -> std::process::Output {
        ...
        cmd.arg("subprocess_env_probe")
            .arg("--nocapture")
            .env("SNAPFZZ_SEAL_SANDBOX_TEST_MODE", mode)
    ...
    fn subprocess_env_probe() {
        let Ok(mode) = std::env::var("SNAPFZZ_SEAL_SANDBOX_TEST_MODE") else {
            return;
        };

        match mode.as_str() {
            "new_uses_env" => {
                let expected = std::env::var("SNAPFZZ_SEAL_EXPECTED").expect("expected path env");
                let provisioner = DockerBackend::new();
    ...
    fn sandbox_provisioner_new_uses_explicit_docker_bin_env() {
        ...
        let output = run_env_probe(
            ...
            &[
                ...
                ("SNAPFZZ_SEAL_EXPECTED", expected.as_str()),
            ],
        );
    ...
    fn provisioner_reports_missing_docker() {
        let provisioner = DockerBackend { docker_bin: None };
        let err = provisioner
            .require_docker()
    ...
    fn default_matches_new() {
        let defaulted = DockerBackend::default();
        let constructed = DockerBackend::new();
    ...
    async fn provision_succeeds_with_stubbed_docker() {
        ...
        assert_eq!(
            handle.backend_data.get("container_id").map(String::as_str),
            Some("container-xyz")
        );
    ...
    #[tokio::test]
    async fn provision_includes_security_flags_in_docker_args() {
        let dir = temp_path("sandbox-provision-security-flags");
        ...
        assert!(logged.contains("--security-opt"));
        assert!(logged.contains("--cap-drop"));
        assert!(logged.contains("--read-only"));
        assert!(logged.contains("--tmpfs"));
        assert!(logged.contains("--pids-limit"));
    ...
    async fn collect_fingerprint_returns_docker_snapshot() {
        ...
        assert_eq!(
            snapshot.runtime,
            snapfzz_seal_fingerprint::model::RuntimeKind::Docker
        );
    ...
    async fn copy_into_sandbox_requires_container_id() {
        ...
        let err = copy_into_sandbox(&provisioner, &handle, Path::new("/tmp/a"), "/tmp/b")
            .await
    ...
    async fn copy_into_sandbox_runs_docker_cp() {
        ...
        copy_into_sandbox(&provisioner, &handle, &host_file, "/tmp/agent")
            .await

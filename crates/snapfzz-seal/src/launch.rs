...
pub fn run(cli: Cli) -> Result<(), Box<dyn std::error::Error>> {
    let launcher_cli = snapfzz_seal_launcher::Cli {
        payload: cli.payload,
        fingerprint_mode: match cli.fingerprint_mode {
            FingerprintMode::Stable => snapfzz_seal_launcher::FingerprintMode::Stable,
            FingerprintMode::Session => snapfzz_seal_launcher::FingerprintMode::Session,
        },
        user_fingerprint: cli.user_fingerprint,
        verbose: cli.verbose,
    };
    snapfzz_seal_launcher::run(launcher_cli).map_err(Into::into)
}

...
mod tests {
    ...
    fn resolve_launcher_path_reads_env_when_cli_missing() {
        if std::env::var_os("AGENT_SEAL_TEST_RESOLVE_FROM_ENV_CHILD").is_some() {
        ...
        let output = std::process::Command::new(current_exe)
            ...
            .env("AGENT_SEAL_TEST_RESOLVE_FROM_ENV_CHILD", "1")
            .env("AGENT_SEAL_LAUNCHER_PATH", "/tmp/from-env-only")
            .output()
    ...
    fn resolve_launcher_path_errors_when_missing_everywhere() {
        if std::env::var_os("AGENT_SEAL_TEST_RESOLVE_MISSING_CHILD").is_some() {
        ...
        let output = std::process::Command::new(current_exe)
            ...
            .env("AGENT_SEAL_TEST_RESOLVE_MISSING_CHILD", "1")
            .env_remove("AGENT_SEAL_LAUNCHER_PATH")
            .output()
            .expect("child test process should execute");
    ...
    fn run_errors_when_launcher_missing() {
        if std::env::var_os("AGENT_SEAL_TEST_RUN_MISSING_LAUNCHER_CHILD").is_some() {
            let cli = Cli {
                project: PathBuf::from("/tmp/project"),
                user_fingerprint: "11".repeat(32),
                sandbox_fingerprint: "22".repeat(32),
        ...
        let output = std::process::Command::new(current_exe)
            ...
            .env("AGENT_SEAL_TEST_RUN_MISSING_LAUNCHER_CHILD", "1")
            .env_remove("AGENT_SEAL_LAUNCHER_PATH")
            .output()
            .expect("child test process should execute");

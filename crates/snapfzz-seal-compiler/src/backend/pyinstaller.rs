...
mod tests {
    ...
    #[test]
    fn pyinstaller_backend_name_and_detection() {
        let project_dir = unique_temp_dir("snapfzz-seal-pyinstaller-backend-detect");
        let backend = PyInstallerBackend;
        assert_eq!(backend.name(), "pyinstaller");
        assert!(!backend.can_compile(&project_dir));
